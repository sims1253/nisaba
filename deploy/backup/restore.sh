...
if [ -d "${SRC}/seaweedfs" ]; then
    ...
    if ! docker run --rm -i --network "$obj_net" --entrypoint /bin/sh         -e AWS_ACCESS_KEY_ID="${NISABA_S3_ADMIN_KEY}"         -e AWS_SECRET_ACCESS_KEY="${NISABA_S3_ADMIN_SECRET}"         -v "${SRC}/seaweedfs:/in:ro"         amazon/aws-cli:2.25.28 -c '
